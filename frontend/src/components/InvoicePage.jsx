...
import { FaDownload, FaArrowLeft, FaCheckCircle, FaShoppingBag } from 'react-icons/fa'
...
const Invoice = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await fetch(`${SummaryApi.getOrderDetails.url}?orderId=${orderId}`, {
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center font-medium">Loading Invoice...</div>
  }

  if (!orderData) {
    return <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center font-medium">Order not found!</div>
  }

  // Extracting payment details safely based on common backend schemas
  const paymentMethod = orderData.paymentDetails?.payment_method_type?.[0] || orderData.paymentMethod || 'Online / Prepaid';
  const paymentStatus = orderData.paymentDetails?.payment_status || orderData.paymentStatus || 'Paid';
    ...
    <div className="min-h-screen bg-slate-950 py-10 px-4 sm:px-6">
      ...
      >
        ...
        <div className="flex justify-between items-start border-b-2 border-purple-600 pb-6 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Smart Shopping Platform</p>
            </div>
          </div>
          <div className="text-right">
            <h2 className="text-xl font-extrabold text-slate-800 uppercase tracking-wide">Tax Invoice</h2>
            <p className="text-xs text-slate-500 mt-1">Invoice No: <span className="font-semibold text-slate-700">{orderId?.slice(-8).toUpperCase()}</span></p>
            <p className="text-xs text-slate-500">Date: <span className="font-semibold text-slate-700">{new Date(orderData.createdAt).toLocaleDateString()}</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
          <div>
            <h3 className="text-xs font-bold text-purple-600 uppercase tracking-wider mb-2">Sold By:</h3>
            <p className="font-bold text-slate-800">Snapkart Retail Pvt Ltd.</p>
            <p className="text-slate-600 text-xs mt-0.5">E-Commerce Tech Park, Sector 62</p>
            <p className="text-slate-600 text-xs">GSTIN: 07AABCS1234F1Z5</p>
            <p className="text-slate-600 text-xs">Support: support@snapkart.com</p>
          </div>
          <div>
            <h3 className="text-xs font-bold text-purple-600 uppercase tracking-wider mb-2">Shipping Address:</h3>
            <p className="font-bold text-slate-800">{orderData.shippingAddress?.name || "Customer"}</p>
            <p className="text-slate-600 text-xs mt-0.5">{orderData.shippingAddress?.address}</p>
            <p className="text-slate-600 text-xs">{orderData.shippingAddress?.city}, {orderData.shippingAddress?.state} - {orderData.shippingAddress?.pincode}</p>
            <p className="text-slate-600 text-xs mt-1 font-medium">Phone: {orderData.shippingAddress?.mobile || orderData.shippingAddress?.phone}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6 mb-8 text-xs border-b border-slate-200 pb-4">
          <div>
            <span className="font-bold text-slate-400 uppercase tracking-wider block mb-1">Payment Method:</span>
            <span className="font-semibold text-slate-800 uppercase bg-slate-100 px-2.5 py-1 rounded-md inline-block">
              {paymentMethod}
            </span>
          </div>
          <div>
            <span className="font-bold text-slate-400 uppercase tracking-wider block mb-1">Payment Status:</span>
          ...
          <tbody className="divide-y divide-slate-100 text-sm">
            {orderData.productDetails?.map((item, index) => (
              <tr key={index}>
                ...
                <td className="py-4 px-4 text-center text-slate-600 font-semibold">{item.quantity || 1}</td>
                <td className="py-4 px-4 text-right text-slate-600">₹{item.price?.toLocaleString()}</td>
                <td className="py-4 px-4 text-right font-bold text-slate-800">₹{(item.price * (item.quantity || 1)).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end mb-8">
          <div className="w-72 space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
            ...
            <div className="flex justify-between text-slate-900 font-extrabold text-base border-t border-slate-200 pt-2 mt-2">
              ...
              <span className="text-purple-600">₹{orderData.totalAmount?.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-200 pt-6 text-center text-xs text-slate-400 space-y-1">
          ...
          <p className="font-semibold text-slate-600">Thank you for shopping with Snapkart! Visit again.</p>
        </div>
      </div>
    </div>
  )
